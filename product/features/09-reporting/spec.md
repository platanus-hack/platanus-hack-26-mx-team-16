...
- **Ruta `/r/[token]`, shell público y sistema visual del reporte** → la superficie de renderizado (route-groups, layout, design system, breakpoints) vive en [13-frontend](../13-frontend/spec.md); este documento define el contenido/contrato que esa superficie pinta.
- **Semántica de los gauges y del grado A–F** (cómo se calculan los dos sub-scores y el grado, el cap a C por cobertura parcial) → ver [07-scoring](../07-scoring/spec.md). Aquí solo se especifica cómo se *renderizan*.
- **Contrato del endpoint `POST /scans/{id}/share` y `GET /r/[token]`** (forma del request/response, status codes a nivel HTTP) → ver [12-api](../12-api/spec.md). Aquí se especifica la semántica del token y la redacción del payload, no la forma del endpoint.
- **Inventario de superficie agéntica y el "star finding" agéntico** (cómo se detecta, `agentic_status`) → ver [03-agentic-surface](../03-agentic-surface/spec.md); aquí solo cómo se presenta en el reporte.
- **Tendencia histórica** (`dedupe_key`, `first_seen`/`last_seen`, comparación entre escaneos) → la mecánica de dedupe a nivel site vive en [06-data-model](../06-data-model/spec.md) y [08-ranking-watchlists](../08-ranking-watchlists/spec.md); aquí solo su presentación.
...
- **Párrafo "Owliver te explica"** — redactado por Opus (ver [05-agent-team](../05-agent-team/spec.md)): explica en lenguaje llano qué encontramos y por qué importa, **sin jerga**.
- **Top 3 riesgos priorizados** con su **impacto de negocio** (qué pasaría si se explota, en términos que entiende quien decide).
- **Inventario de superficie agéntica detectada** — qué chatbots / widgets de IA tiene el sitio (ver [03-agentic-surface](../03-agentic-surface/spec.md)).
- **Badges de estado cuando aplique:**
  - **"IA detectada, sin auditar"** cuando `agentic_status = detected_not_tested` — el sitio tiene superficie agéntica que no pudo probarse (ver [07-scoring](../07-scoring/spec.md)).
  - **"cobertura parcial"** cuando el grado quedó capado en **C** por cobertura insuficiente del escaneo (ver [07-scoring](../07-scoring/spec.md)).
...
La forma del endpoint vive en [12-api](../12-api/spec.md); la semántica del token es:
