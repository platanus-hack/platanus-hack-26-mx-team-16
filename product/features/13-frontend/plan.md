...
> propio que sirve las superficies anónimas virales (ranking, reporte
...
> `/login` (Google, §F10) y **reescribe `/` como el ranking** RSC. El form
...
|---|---|---|
| `/` ranking | **RSC** `serverHttp.get` (+ react-query client para "cargar más"/filtros vía `route.ts`) | `GET /ranking?country=mx` (cursor) |
...
- **Responsive (un breakpoint `md`):** leaderboard tabla→cards; reporte 2-col→stack;
  ...
  obligatoria para ranking, Theater y Reporte.
...
1. **Piel + deps (0–2):** tokens `--ow-*`/`--soc-*`/grades en `globals.css`;
   ...
   Owliver; mover login→`/login`, `/`→ranking stub; ampliar `proxy.ts`
...
1. **`/` se reescribe como ranking; login se mueve a `/login` (Google).**
