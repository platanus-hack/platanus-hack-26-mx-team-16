...
scans(id UUID PK, site_id, level ENUM(basico,intermedio,avanzado),
      ...
      summary jsonb NULL,                -- ExecutiveSummary serializado (narrativa + top_risks) de Opus (§3.2; 05 persiste, 09 lee)
...
scan_events(id, scan_id, seq int, ts, type, agent, tool NULL,
            severity NULL, message, payload jsonb, progress int NULL)
...
- **Scoring (calculado en Python, ver [07-scoring](../07-scoring/spec.md)):**
  ...
  - `summary jsonb NULL` — `ExecutiveSummary` serializado (narrativa + `top_risks`) que redacta el orquestador Opus. Lo persiste el worker (ver [05-agent-team](../05-agent-team/spec.md)) y lo lee el reporte (ver [09-reporting](../09-reporting/spec.md) D1).
...
- `type` — `chatbot | prompt_input | search_ai`.
...
- `progress int NULL` — progreso 0..100 (eventos `phase`/`score`; barra del header en el live-view, ver [10-realtime-live-view](../10-realtime-live-view/spec.md)). [10-realtime-live-view](../10-realtime-live-view/spec.md); aquí solo se fija la forma de la tabla y la garantía de orden.
