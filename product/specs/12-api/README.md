...
`public_reports`, `watchlist`) son propiedad de [06-data-model](../06-data-model/README.md);
el cálculo de scores expuesto por estos endpoints es propiedad de
[07-scoring](../07-scoring/README.md).

## Autenticación (`/auth/*`) — contrato HTTP

El **flujo** completo de magic-link (envío, canje, las 4 pantallas, JWT y cookie)
es propiedad de [11-auth-magic-link](../11-auth-magic-link/README.md). Aquí se
...
SSE (`EventSource` no admite headers custom; ver [10-realtime-live-view](../10-realtime-live-view/README.md)).
...
**segundo ataque no consentido** (ver [01-legal-ethics](../01-legal-ethics/README.md)).
...
- El reporte público se sirve **solo vía token** en `/r/{token}` (con exploits
  redactados, §11.3), **nunca** vía `/scans/{id}`. Ver [09-reporting](../09-reporting/README.md).
...
solo uso `?stream_token=`); ver [10-realtime-live-view](../10-realtime-live-view/README.md).
...
- `status` (enum, incluye `cancelled` y `partial`; ver [06-data-model](../06-data-model/README.md)).
- `web_score` / `agentic_score` (cálculo en [07-scoring](../07-scoring/README.md)).
...
cada finding y su `dedupe_key` son propiedad de [06-data-model](../06-data-model/README.md).

### `GET /scans/{id}/stream` — SSE live view

Contrato del endpoint SSE; la semántica de **replay-then-tail** y el esquema de
eventos son propiedad de [10-realtime-live-view](../10-realtime-live-view/README.md).
...
- Token válido → reporte **redactado**: capa ejecutiva + findings técnicos **sin
  ...
  [09-reporting](../09-reporting/README.md) (§11.3).
