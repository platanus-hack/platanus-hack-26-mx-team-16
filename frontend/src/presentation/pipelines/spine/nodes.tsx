...
export function SoloNode({
  ...
  return (
    <div
      className={`pe-card v-${ui.nodeStyle}${selected ? " is-sel" : ""}`}
      style={{
        position: "absolute",
        left: block.x,
        top: block.y,
      ...
      data-pe-measure={s.id}
      onClick={(e) => {
        e.stopPropagation();
        onSelect({ stageId: s.id, kind: phase.kind, id: phase.realId });
      }}
    >
      <Port side="top" />
      ...
      <div className="pe-body">
        ...
        <div className="pe-main">
          <div className="pe-titlerow">
            <span
              className="pe-num"
              style={{ color: accent.text, background: accent.soft }}
            >
              {s.num}
            </span>
      ...
      <div className="pe-body">
        ...
        <div className="pe-main">
          ...
          <div className="pe-grpmeta">
            <span
              className="pe-pilltag"
              style={{ color: accent.text, borderColor: accent.line }}
            >
              {s.phases.length} fases · {s.atomic ? "par atómico" : "grupo"}
            </span>
            <span className="pe-minidots">
              {s.phases.map((p) => (
                <i key={p.kind} style={{ background: accent.line }} />
              ))}
            </span>
          </div>
        </div>
...
export function GroupBox({
  ...
  return (
    <div
      className={`pe-group v-${ui.nodeStyle}${selected ? " is-sel" : ""}`}
      style={{
        position: "absolute",
        left: block.x,
        top: block.y,
    ...
    >
      ...
      <div
        className="pe-ghead"
        style={{ height: ui.density === "compacto" ? 42 : 50 }}
      >
        <Glyph icon={s.icon} accent={accent} ui={ui} />
        <span
          className="pe-num"
...
export function InnerNode({
  ...
  return (
    <div
      ...
      data-pe-measure={node.id}
      onMouseDown={(e) => e.stopPropagation()}
      onClick={(e) => {
        e.stopPropagation();
        onSelect({ stageId: node.stageId, kind: p.kind, id: p.realId });
      }}
    >
      ...
      <div className="pe-imain">
        ...
        <div className="pe-ikind">{p.kind}</div>
      </div>
      {p.optional && (
        <button
          type="button"
          className="pe-sw"
          data-on={disabled ? "0" : "1"}
          title={disabled ? "Activar fase" : "Desactivar fase"}
          onMouseDown={(e) => e.stopPropagation()}
          onClick={(e) => {
            e.stopPropagation();
            onToggleOptional(p.kind);
          }}
        >
          <i />
        </button>
...
export function PipeEdges({ layout, ui }: { layout: Layout; ui: Appearance }) {
  ...
  layout.blocks.forEach((b) => {
    if (b.inner) b.inner.edges.forEach((e) => innerEdges.push(e));
  });
  return (
    <svg
      className="pe-edges"
    ...
    >
      ...
      {layout.edges.map((e, i) => (
        <path
          key={`e${i}`}
          d={edgePath(e, ui.edgeStyle)}
          fill="none"
          stroke="var(--pe-edge)"
          strokeWidth="1.8"
          markerEnd="url(#peArr)"
          strokeDasharray={e.crossScope ? "5 4" : "none"}
          opacity={e.crossScope ? 0.7 : 1}
        />
      ))}
      {innerEdges.map((e, i) => (
        <path
