...
- [x] **F2 — hecho (sin dependencias nuevas).** `globals.css`: utilidades `ease-emphasized*`, keyframes `m3-enter` / `m3-pulse-once` / `m3-wavy` (+ tokens `--animate-*` → `animate-m3-enter` etc.), y **guarda global `@media (prefers-reduced-motion: reduce)`**. Nuevos archivos: `application/hooks/use-reduced-motion.ts`, `application/lib/motion.ts` (duraciones/easing/`easeFn`/`spring` presets espejo de DESIGN.md), y `presentation/components/common/`: `count-up.tsx` (reveal del grado/score), `reveal.tsx` (fade + slide-up con `delay` para stagger), `wavy-progress.tsx` (indicador **wavy** M3, SVG dependency-free). Todos hacen branch en `useReducedMotion()`. Opcional a futuro: instalar `motion`/Framer para springs físicos (los presets `spring` ya están listos para esa lib).
- [~] **F3 — en progreso.** `sidebar` → items **pill** (activo tonal vía `sidebar-accent`). `dashboard-view` → cards envueltas en `Reveal` (stagger 0/60/120/180) + `CountUp` en el contador de miembros. El resto (members/roles/profile/settings/api-keys, auth, estados forbidden/unassigned) **hereda** los tokens + primitivos M3 ya migrados; quedan toques opcionales (aplicar `Reveal`/`CountUp` donde aporte; charts cuando se agreguen). `npx tsc --noEmit` **sin errores** tras F0–F3.
