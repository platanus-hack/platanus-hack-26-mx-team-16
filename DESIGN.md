...
A `#5FC487` · B `#9FD06E` · C `#ECCB68` · D `#F0A05E` · E `#EC7E63` · F `#E0635F`. Softened to pastel, but F still reads clearly as the worst — the failing-grade red wall is gentler, not gone. This ramp is the **only** place semantic green/amber/red appears as *grade* (chips, gauges, leaderboard rows, the failing-grade "F" wall). It is intentionally **not** themed away by M3 — a grade's color is data. (It is deliberately distinct from the brand's violet/teal so a high grade never reads as "brand color.")
